...
#     with SessionLocal() as session:
#         # Строим запрос
#         stmt = select(func.strftime('%Y-%m', mytrade_results_table.c.date_sale).label('sale_month'),
#                       func.sum(mytrade_results_table.c.net_profit).label('total_net'),
#                       func.count(mytrade_results_table.c.id).label('deals_count')).group_by(
#             func.strftime('%Y-%m', mytrade_results_table.c.date_sale))
#         result_week = session.execute(stmt)
#         rows = result_week.mappings().all()
#         return rows
#
